...
# فعّل هذا لتجاوز OpenAI Images واستعمال صور Placeholder محلية
SEED_PLACEHOLDERS: bool = True

# إن أردت استخدام OpenAI Images لاحقًا، ضع هذا False واجهّز المفتاح/التحقّق
client = None
...
# ================== نواة توليد صورة لمشهد ==================
def _generate_openai_image(prompt: str, idx: int, size: str = "1024x1024") -> str:
    """
    توليد صورة عبر OpenAI (عند توافره). سيرفع استثناء لو فشل.
    """
    if client is None:
    ...
    # ملاحظة: يلزم حساب/منظمة مُوثّقة لاستخدام gpt-image-1
    resp = client.images.generate(
        model="gpt-image-1",
        prompt=full_prompt,
        size=size,
        quality="standard",  # لا تغيّرها إلى 'auto' أو 'high' هنا
        n=1,
    )
...
def generate_image_for_scene(
    ...
    if SEED_PLACEHOLDERS:
        return _draw_placeholder(scene_description, index, size=tuple(map(int, size.split("x"))))

    # استخدام OpenAI ثم فولباك
    try:
...
if _name_ == "_main_":
    ...
    out = generate_images(demo_script)
