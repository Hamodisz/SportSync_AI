...
class BaseSink:
    name = "base"
    def send(self, event: Dict[str, Any]) -> bool: raise NotImplementedError

class DiskSink(BaseSink):
    name = "disk"
    def _init_(self, path: str = "./data/events.jsonl"):
        self.path = path
...
class GSheetsSink(BaseSink):
    name = "gsheets"
    def _init_(self, sheet_id: str, svc_json: Optional[str]):
    ...
    def _ensure_ws(self):
        ...
        try:
            ...
            ws.append_row([
                "ts_utc","event_type","user_id","session_id","lang","model","app_version","payload_json"
            ])
        return ws
...
class DataPipe:
    ...
    def send(self, event_type: str, payload: Dict[str, Any],
        ...
        if not self.sinks: self._build_sinks()
        evt = {
